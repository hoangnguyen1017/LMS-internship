...
def home(request):
    ...
    if request.user.is_authenticated:
        try:
            ...
            # Kiểm tra vai trò tạm thời
            temporary_role_id = request.session.get('temporary_role')
            if temporary_role_id:
                user_role = Role.objects.get(id=temporary_role_id)

            # Xác định module hiển thị dựa trên vai trò
            if request.user.is_superuser:
                # Nếu là superuser, chỉ hiển thị module theo vai trò tạm thời nếu có
                if temporary_role_id:
                    user_role = Role.objects.get(id=temporary_role_id)
            ...
            messages.error(request, "Temporary role does not exist.")
            modules = Module.objects.none()
