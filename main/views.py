        ...
        if confirmation_code_form.is_valid():
            ...
            if code == str(request.session.get('confirmation_code')):
                ...
                if user_exists(email):
                    messages.error(request, "This email is already in use. Please login or use another email!")
                    return redirect('main:register_email')

                # Lưu email vào session để sử dụng sau
                request.session['email_verified'] = email

                messages.success(request, "Email verification successful! Please fill in registration information.")
...
def toggle_2fa(request, pk):
    ...
    if user.is_superuser:
        messages.warning(request, "Tài khoản superuser không cần bật/tắt 2FA.")
        return redirect('user:user_detail', pk=user.pk)

    # Nếu tính năng 2FA đang bật và người dùng muốn tắt
    if profile.two_fa_enabled:
        # Tạo mã xác nhận ngẫu nhiên và lưu vào session
        token = ''.join(random.choices(string.ascii_letters + string.digits, k=32))
        expiration_time = timezone.now() + timedelta(minutes=10)  # Mã có hiệu lực trong 10 phút
        ...
        # Gửi email xác nhận
        send_mail(
            'Xác nhận tắt bảo mật 2 lớp (2FA)',
            f'Bạn có muốn tắt bảo mật 2 lớp (2FA) không?\n\n'
            f'Vui lòng nhấn vào liên kết dưới đây để xác nhận:\n\n'
            f'Xác nhận: {confirm_link}',
            'admin@yourdomain.com',
            [user.email],
            fail_silently=False,
        )

        messages.info(request, "Một email xác nhận đã được gửi. Vui lòng kiểm tra email của bạn để tắt 2FA.")
        return redirect('user:user_detail', pk=user.pk)

    # Nếu tính năng 2FA chưa được bật
...
def confirm_2fa(request, token):
    # Kiểm tra mã xác nhận trong session
    ...
    if session_token != token or not session_expiration:
        messages.error(request, "Mã xác nhận không hợp lệ.")
        return redirect('user:user_list')  # Hoặc redirect về trang khác nếu cần

    # Sử dụng datetime.fromisoformat thay vì timezone.fromisoformat
    expiration_time = datetime.fromisoformat(session_expiration)

    if timezone.now() > expiration_time:
        messages.error(request, "Mã xác nhận đã hết hạn.")
        return redirect('user:user_list')  # Hoặc redirect về trang khác nếu cần

    # Lấy profile của người dùng
    profile = Profile.objects.get(user=request.user)

    # Kiểm tra nếu tính năng 2FA đã được tắt trước đó, không làm gì nữa
    if not profile.two_fa_enabled:
        messages.info(request, "Tính năng 2FA đã được tắt trước đó.")
    ...
    # Xóa mã xác nhận khỏi session ngay lập tức
    del request.session['two_fa_token']
    del request.session['two_fa_expiration']

    messages.success(request, "Tính năng 2FA đã được tắt.")

    # Quay lại trang chi tiết người dùng
    return redirect('user:user_detail', pk=request.user.pk)
